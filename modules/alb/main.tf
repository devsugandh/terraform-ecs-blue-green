...
resource "aws_lb" "this" {
  ...
  subnets            = var.aws_alb_subnet_ids
...
resource "aws_lb_listener" "lb_http_listener" {
  ...
  dynamic "default_action" {
    ...
    # Defaults forward action if action_type not specified
    content {
      type             = lookup(default_action.value, "action_type", "forward")
      target_group_arn = contains([null, "", "forward"], lookup(default_action.value, "action_type", "")) ? aws_lb_target_group.target_group[lookup(default_action.value, "target_group_index", count.index)].id : null
...
resource "aws_lb_listener" "lb_https_listener" {
  ...
  dynamic "default_action" {
    ...
    content {
      type             = "forward"
      target_group_arn = aws_lb_target_group.target_group[lookup(default_action.value, "target_group_index", count.index)].id
    }
  }
  lifecycle {
    ignore_changes = [
...
resource "aws_lb_listener_certificate" "https_listener" {
  count = var.create_alb ? length(var.extra_ssl_certs) : 0

  listener_arn    = aws_lb_listener.lb_https_listener[var.extra_ssl_certs[count.index]["https_listener_index"]].arn
  certificate_arn = var.extra_ssl_certs[count.index]["certificate_arn"]
}

resource "aws_lb_listener_rule" "https_listener_rule" {
  count = var.create_alb ? length(var.https_listener_rules) : 0

  listener_arn = aws_lb_listener.lb_https_listener[lookup(var.https_listener_rules[count.index], "https_listener_index", count.index)].arn

...
resource "aws_eip" "eip_nat_gateway" {
  count = var.create_private_subnet && var.single_nat_gateway ? 1 : length(var.aws_availability_zones)
...
resource "aws_nat_gateway" "nat_gateway" {
  count = var.create_private_subnet ? (var.single_nat_gateway ? 1 : length(var.aws_availability_zones)) : 0

  allocation_id = element(aws_eip.eip_nat_gateway.*.id, var.single_nat_gateway ? 0 : count.index)
